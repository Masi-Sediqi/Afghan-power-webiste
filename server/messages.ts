...
export function createMessageStore(file=defaultFile){return {
  ...
  async create(input:ContactMessageInput){
    const name=clean(input.name),phone=clean(input.phone),message=clean(input.message),email=clean(input.email),division=clean(input.division),service=clean(input.service),subject=clean(input.subject)
    const requestType=input.requestType==='demo'?'demo':'contact', company=clean(input.company), preferredDate=clean(input.preferredDate), preferredTime=clean(input.preferredTime), meetingType=clean(input.meetingType), timeZone=clean(input.timeZone)
    if(!name)throw new Error('Full name is required.'); if(!phone)throw new Error('Phone number is required.'); if(!message)throw new Error('Message is required.'); if(email&&!email.includes('@'))throw new Error('Enter a valid email address.')
    if(requestType==='demo'){ if(!service)throw new Error('Choose a product or service.'); if(!preferredDate)throw new Error('Choose a preferred demo date.'); if(!preferredTime)throw new Error('Choose a preferred demo time.'); if(!meetingType)throw new Error('Choose a meeting type.') }
    const now=new Date().toISOString(); const item:ContactMessageRecord={id:randomUUID(),name,phone,email,division,service,subject,message,requestType,company,preferredDate,preferredTime,meetingType,timeZone,status:'new',createdAt:now,updatedAt:now}; const all=await readAll(file);all.push(item);await writeAll(file,all);return item
  },
